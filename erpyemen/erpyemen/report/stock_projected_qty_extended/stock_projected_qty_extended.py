...
def execute(filters=None):
	...
	for bin in bin_list:
		...
		try:
			...
			for u in uoms:
				if remaining_qty >= u.conversion_factor:
					units = remaining_qty // u.conversion_factor
					remaining_qty = remaining_qty % u.conversion_factor
					parts.append(f"{units} {u.uom}")

			if remaining_qty > 0:
				parts.append(f"{remaining_qty} {item.stock_uom}")
...
def get_columns():
	return [
		...
		{
		"label": _("Qty Based on UOM"),
		"fieldname": "qty_based_on_uom",
		"fieldtype": "Data",
		"width": 150,
		},
		{
